...
const CourseCard = ({ item }) => {
  ...
  const handleEnrollClick = async (item) => {
    try {
      ...
      // console.log('newcoursedata', JSON.stringify(data));
    } catch (error) {
      console.log(error);
    }
  };
        ...
        <div className="single__course__item course-card" key={item.id}>
          <>
            ...
            <div className="course__details">
              <h6 className="course__title mb-4"><strong>{item?.attributes?.title}</strong></h6>

              <div className=" d-flex justify-content-between align-items-center">
                <p className="lesson d-flex align-items-center gap-1">
                  <i className="ri-book-open-line"></i> {item?.attributes?.lesson}{" "}
                  Lessons
                </p>
